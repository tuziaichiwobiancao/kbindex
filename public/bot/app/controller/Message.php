        ...
        switch ($this->update->getMessage()->getChat()->getType()){
            ...
            case "private":
                ...
                if($this->getSystemConfig("issearch")){
                    $arr = [
                        "keyword" => $this->update->getMessage()->getText(),
                        "uid" => $this->update->getMessage()->getChat()->getId(),
                        "username" => $this->update->getMessage()->getChat()->getUsername(),
                        "nick" => $this->update->getMessage()->getFrom()->getFirstName(),
                        "gid" => null,
                        "gusername" => null,
                        "gnick" => null,
                        "type" => 0
                    ];
                    $this->writeSearchIndex($arr);
            ...
            case "supergroup":
                ...
                try {
                    ...
                    $arr = [
                        "keyword" => $this->update->getMessage()->getText(),
                        "uid" => $this->update->getMessage()->getFrom()->getId(),
                        "username" => $this->update->getMessage()->getFrom()->getUsername(),
                        "nick" => $this->update->getMessage()->getFrom()->getFirstName(),
                        "gid" => $this->update->getMessage()->getChat()->getId(),
                        "gusername" => $this->update->getMessage()->getChat()->getUsername(),
                        "gnick" => $this->update->getMessage()->getChat()->getTitle(),
                        "type" => 1
                    ];
                    $this->writeSearchIndex($arr);
                        $this->bot->sendMessage($this->update->getMessage()->getChat()->getId(), $content->getContent($this->update->getMessage()->getText()), "HTML", true, $this->update->getMessage()->getMessageid(), $content->getButton($this->update->getMessage()->getText()));
