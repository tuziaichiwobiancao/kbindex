...
class Contents extends Base
{
    ...
    public function getContent($text,$page = 1){
        ...
        //dump($this->getSearchIndex($text));exit();
        $resff = $this->getSearchIndex($text);
        
        $content = str_replace("{day}",$resff["data"]["day"],$this->getSystemConfig("deffootadv"));
        $content = str_replace("{weed}",$resff["data"]["weed"],$content);
        $content = str_replace("{moon}",$resff["data"]["moon"],$content);
        $content = str_replace("{year}",$resff["data"]["year"],$content);
        $content = str_replace("{all}",$resff["data"]["all"],$content);
        //获取系统配置中底部内容
        $topadv .= "\n\n".$content;
        //返回拼接好的字符串
        return $topadv;
