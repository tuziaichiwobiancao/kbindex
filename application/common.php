...
if(!function_exists("gopay")) {
    /**
     * USDT支付
     * @param double $money 金额
     * @return array
     */
    function gopay($orderno,$money)
    {
        $amount = $money * (float)get_system_config("rate");
        $notify_url = $_SERVER['REQUEST_SCHEME']."://".$_SERVER['HTTP_HOST']."/api/pay/notify.html";;//Epusdt的异步回调地址，随便，无需管理的话
        $redirect_url = $_SERVER['REQUEST_SCHEME']."://".$_SERVER['HTTP_HOST']."/api/pay/redirect.html";//Epusdt的同步跳转地址,付款成功后跳转到这里
        $order_id = $orderno;//(string)"USDT-" . date("YmdHis", time()) . rand(10000000, 99999999);//生成随机数用于订单号
        $key = get_system_config("paytoken");//Epusdt的自定义密钥
        ...
        $data = json_encode(array('order_id' => $order_id,                           //生成数据包，用到了的数组转json的jsonencode
            ...
            'signature' => $signature));
        $res = curl_request(get_system_config("payurl"), $data, 'post');        //发起Curl请求并获取返回数据到变量
...
if(!function_exists("getIndex")){
    function getIndex()
    {
        $file = $_SERVER['DOCUMENT_ROOT']."/bot/public/index.php";
        $content = file_get_contents($file);
        $content = str_replace("{url}", $_SERVER['REQUEST_SCHEME']."://".$_SERVER['HTTP_HOST'], $content);
        file_put_contents($file,$content);
    }
}

if(!function_exists("getThink")){
    function getThink(){
        $file = $_SERVER['DOCUMENT_ROOT']."/bot/think";
        $content = file_get_contents($file);
        $content = str_replace("{url}", $_SERVER['REQUEST_SCHEME']."://".$_SERVER['HTTP_HOST'], $content);
        file_put_contents($file,$content);
    }
