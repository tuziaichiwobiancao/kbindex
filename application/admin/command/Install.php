...
class Install extends Command
{
    ...
    protected function installation($mysqlHostname, $mysqlHostport, $mysqlDatabase, $mysqlUsername, $mysqlPassword, $mysqlPrefix, $adminUsername, $adminPassword, $adminEmail = null, $siteName = null)
    {
        ...
        if(!$this->request->post("licenses")){
            throw new Exception(__("请填写授权码"));
        }
        if(!$this->request->post("username")){
            throw new Exception(__("请填写TG用户名"));
        }
        ...
        //修改授权
        $instance->name('config')->where('name', 'licenses')->update(['value' => $this->request->post('licenses')]);
        $instance->name('config')->where('name', 'username')->update(['value' => $this->request->post('username')]);
        // 修改后台入口
        $adminName = '';
